...
export default defineConfig((/* ctx */) => {
  return {
    ...
    devServer: {
      ...
      port: 9000,
      headers: {
        'Content-Security-Policy': "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline'; img-src 'self' data: http://127.0.0.1:8000; font-src 'self' data:; connect-src 'self' ws://localhost:9000 http://127.0.0.1:8000;"
      },
      proxy: {
        // Правило для картинок: перенаправляет /media на бэк как есть
        '/media': {
          target: 'http://127.0.0.1:8000',
          changeOrigin: true,
          secure: false
        },
        // Твое старое правило для API
        '/api': {
          target: 'http://127.0.0.1:8000',
          changeOrigin: true,
          secure: false
